...
export const BoardCanvas: React.FC<BoardCanvasProps> = ({
  ...
  return (
    <div id="board-canvas-container" className={`relative flex-1 ${activeTheme.canvasBg} transition-all duration-500 overflow-hidden flex flex-col`}>
      ...
      {/* Visual Tag Filter Management Bar */}
      <div className="px-4 py-1.5 bg-slate-950/40 backdrop-blur-md border-b border-white/10 flex items-center justify-between gap-3 text-xs overflow-x-auto custom-scrollbar shrink-0">
        <div className="flex items-center gap-1.5 shrink-0">
          <Tag className="w-3.5 h-3.5 text-indigo-400" />
          {selectedTagFilter && (
            <button
              onClick={() => setSelectedTagFilter(null)}
              className="px-2 py-0.5 rounded-md bg-indigo-500 text-white font-semibold text-[10px] flex items-center gap-1 shadow-sm"
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto py-0.5">
          {allBoardTags.map((tag) => {
            ...
            return (
              <button
                ...
                className={`px-2 py-0.5 rounded-md text-[11px] font-semibold border transition-all shrink-0 flex items-center gap-1 ${
                  isSelected 
                    ? 'ring-1 ring-indigo-400 bg-indigo-500 text-white border-indigo-400 shadow-md' 
          ...
          {allBoardTags.length === 0 && (
            <span className="text-[10px] text-slate-500 italic">No active tag filters</span>
          )}
        </div>
      </div>
